...
GLmol.prototype.parsePDB2 = function(str) {
   ...
   for (var i = 0; i < lines.length; i++) {
      ...
      if (recordName == 'ATOM  ' || recordName == 'HETATM') {
         ...
         resid =  line.substr(22, 5).replace(/ /g,'');
  ...
  'hetflag': hetflag, 'chain': chain, 'resi': resi, 'resid':resid, 'serial': serial, 'atom': atom,
...
GLmol.prototype.drawStrand = function(group, atomlist, num, div, fill, coilWidth, helixSheetWidth, doNotSmoothen, thickness) {
   ...
   for (var i in atomlist) {
      ...
      if ((atom.atom == 'O' || atom.atom == 'CA') && !atom.hetflag) {
         if (atom.atom == 'CA') {
            //console.log(currentResi+1+"-"+atom.resi+"-"+atom.resid+"-"+atom.chain)
            if (currentChain != atom.chain ) { //  || currentResi + 1 != atom.resi  // don't know what this is for..
...
GLmol.prototype.getResidueByIdAndChain = function(atomlist, resi, chain) {
   var ret = [];
   for (var i in atomlist) {
      var atom = this.atoms[atomlist[i]]; if (atom == undefined) continue;

      if ((resi == atom.resi || resi == atom.resid)  && chain.indexOf(atom.chain) != -1 && atom.atom != "N" && atom.atom != "C" && atom.atom != "O") ret.push(atom.serial);
      if ((resi == atom.resi || resi == atom.resid) && chain.indexOf(atom.chain) != -1 && atom.atom == "N" && atom.resn == "PRO") ret.push(atom.serial);
   }
   return ret;
};
...
GLmol.prototype.colorByChain = function(atomlist, colorSidechains) {
   ...
   for (var i in atomlist) {
      ...
      if (colorSidechains || atom.atom == 'CA' || atom.atom == 'O3\'') {
         ...
         /////// Added by Franco
         if (atom.chain == bufferChain)
         {
            color.setHSV( colorSidechains[counter], 1, 0.9)
         }
         else
         {
            bufferChain = atom.chain
            counter++;
            color.setHSV( colorSidechains[counter], 1, 0.9)  
         }
         atom.color = color.getHex();
...
GLmol.prototype.zoomInto = function(atomlist, keepSlab) {
   ...
   if (!keepSlab) {
      this.slabNear = -maxD / 1; // by franco, default was 1.9
      this.slabFar = maxD / 3;
   }
...
GLmol.prototype.enableMouse = function() {
   ...
   $('body').bind('mouseup touchend', function(ev) {
      ...
      // console.log(atom.chain + ":" + atom.resn + ":" + atom.resi + ":" + atom.atom, "30", "#ffffff")
